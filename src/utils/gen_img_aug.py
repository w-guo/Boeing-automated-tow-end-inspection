...
def scale(src,label,factor):
    ...
    dst_img = cv2.resize(src,None,fx = factor, fy = factor, interpolation = cv2.INTER_LINEAR)
    dst_label = cv2.resize(label,None,fx = factor, fy = factor, interpolation = cv2.INTER_LINEAR)
...
def chooseSPNoise(src_sub_l1, randTemp):
    ...
    if randTemp < 1/2:
        # Add Salt noise
        coords = [np.random.randint(0, i - 1, int(num_salt)) for i in src_sub_l2.shape]
        src_sub_l2[coords[0], coords[1]] = 255
        # Add Pepper noise
        coords = [np.random.randint(0, i - 1, int(num_pepper)) for i in src_sub_l2.shape]
...
#        temp4 = np.random.random()
#        temp5 = np.random.random()
        #scaling
        src_sub, label_sub = chooseScale(src, label, 1)
        #rotate/flip
        src_sub, label_sub = chooseRotate(src_sub, label_sub, temp2)
        #apply equ hist
        #src_sub = chooseEquHist(src_sub, temp3)
        #apply gamma correction
        #src_sub = chooseGamma(src_sub, temp4)
        #add Gaussian noise
        #src_sub = chooseGaussNoise(src_sub temp5)
        #add salt & pepper noise
        #src_sub = chooseSPNoise(src_sub, temp5)
        cv2.imwrite((folder+'train/src/'+filename+'_%d.jpg' % count), src_sub)
        if np.sum(np.rint(label_sub/255)) == img_w*img_h:
            label_sub = np.zeros_like(label_sub)
        cv2.imwrite((folder+'train/label/'+filename+'_%d.jpg' % count), label_sub)        
        count += 1 
