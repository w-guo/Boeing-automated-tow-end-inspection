...
def find_major_orientations(lines):
    ...
    # 1D clustering
    kde = KernelDensity(bandwidth=2, kernel='gaussian').fit(
        np.array(orit).reshape(-1, 1))
...
def merge_lines(lines):
    ...
    merged_lines_all = [endpoint[0]+endpoint[1]
                        for endpoint in merged_lines_all]
    ...
    if n_l > 1:
        ...
        for i in range(n_l):
            for j in range(i+1, n_l):
                ...
                # condition for connection
                if orit_diff_m[i, j] < 5 and (abs(orit_i) > 85 or abs(orit_i) < 5):
